<?php
/**
 * Diglin GmbH - Switzerland
 *
 * @author      Sylvain Rayé <support at diglin.com>
 * @category    Diglin
 * @package     Diglin
 * @copyright   Copyright (c) Diglin (http://www.diglin.com)
 */

/**
 * Class Diglin_GooglePrint_Block_Adminhtml_Config_Source_Hint
 */
class Diglin_GooglePrint_Block_Adminhtml_Config_Source_Hint
    extends Mage_Adminhtml_Block_Abstract
    implements Varien_Data_Form_Element_Renderer_Interface
{
    /**
     * Render fieldset html
     *
     * @param Varien_Data_Form_Element_Abstract $element
     *
     * @return string
     */
    public function render(Varien_Data_Form_Element_Abstract $element)
    {
        $html = '<p><a href="http://www.diglin.com/?utm_source=magento&utm_medium=extension&utm_campaign=zopim">'
            . '<img style="vertical-align:middle" src="data:image/png;base64,">'
            . '</a> <a href="http://www.diglin.com/?utm_source=magento&utm_medium=extension&utm_campaign=ricento">Diglin GmbH</a> | Rütistrasse 14, 8952 Schlieren - Switzerland | <a href="mailto:support@diglin.com?subject=Support Diglin ricardo.ch Extension">support@diglin.com</a></p>';

        $helper = Mage::helper('diglin_googleprint');

        $buttonAuthorize = $buttonAuthorizeTest = $buttonAuthorizeRevoke = null;

        try {
            if (!$helper->isConfigured()) {
                $buttonAuthorize = $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
                    'label'   => $this->__('Authorize Google Account Access'),
                    'onclick' => "window.open('" . $this->getUrl('*/googleprint_api/authorize') . "', '_self');",
                    'class'   => 'go',
                    'type'    => 'button',
                    'id'      => 'google-account',
                ))
                    ->toHtml();
            }

            if (Mage::getIsDeveloperMode()) {
                $buttonAuthorizeTest = $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
                    'label'   => $this->__('Authorize Google Account Access TEST'),
                    'onclick' => "window.open('" . $this->getUrl('*/googleprint_test/authorize') . "', '_self');",
                    'class'   => 'go',
                    'type'    => 'button',
                    'id'      => 'google-account-test',
                ))
                    ->toHtml();
            }

            if ($helper->getOAuthToken()) {
                $buttonAuthorizeRevoke = $this->getLayout()->createBlock('adminhtml/widget_button')->setData(array(
                    'label'   => $this->__('Revoke Access'),
                    'onclick' => "window.open('" . $this->getUrl('*/googleprint_api/revoke') . "', '_self');",
                    'class'   => 'go',
                    'type'    => 'button',
                    'id'      => 'google-account-test',
                ))
                    ->toHtml();
            }

        } catch (Exception $e) {
            // do nothing just don't display it as the key may be not yet configured.
        }

        return $html
            . '<p>'
            . $buttonAuthorize
            . '&nbsp;&nbsp;'
            . $buttonAuthorizeTest
            . '&nbsp;&nbsp;'
            . $buttonAuthorizeRevoke
            . '&nbsp;&nbsp;<strong>Diglin_GooglePrint Version: '
            . Mage::getConfig()->getModuleConfig('Diglin_GooglePrint')->version
            . ' </strong>'
            . '</p>';
    }
}
